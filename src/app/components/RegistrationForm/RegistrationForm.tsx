...
import { setRegistration } from '@/app/api/actionRegistration';
...
const RegistrationForm: FC = () => {
    ...
    const {toggleFormContent} = useUserAccountContext();
    const onSubmitRegister = async (data: any) => {
        setLoadingReg(true)
        try {
            const dat = await setRegistration(data)
            if (dat?.Result?.Data) {
                // console.log('регистрация прошла успешно', dat)
            } else {
                const errorMassage = dat?.Error?.Message
                alert(errorMassage)
            }
        } catch (error) {
        ...
        <>
            ...
            {successReg ? 
                <div className={style['success-form']}>
                    ...
                    <button onClick={() => {
                        setSuccessReg(false)
                        toggleFormContent('login')
                        reset() 
                    }}>Закрыть</button> 
                </div> :
                <form autoComplete='off' className={`${style["form-register"]} ${style["form-lk"]}`} onSubmit={handleSubmit(onSubmitForm)}>
                <div className={style["form-register__wrapper"]} id="register-form-passager">
                    <label>Номер телефона </label>
                    <input autoComplete='off' type="text" {...register("phone", {
                        required: "Пожалуйста введите номер телефона",


                    })} className={style["phone-register-user"]} placeholder="+375 29 000 45 45" />
                    <ErrorMessage errors={errors} name="phone" />
                    <label>Введите новый пароль </label>
                    <div className={style["password-wrapper"]}>
                        <input autoComplete='off' type={isVissible ? "text" : "password"} {...register("password", { required: "Пожалуйста введите пароль" })} id="password-register-user" placeholder="Пароль" />
                    ...
                    <div className={style["password-wrapper"]}>
                        <input autoComplete='off' type={isVissibleConfirm ? "text" : "password"} {...register("passwordRepeat", { required: "Пожалуйста повторите пароль", })} id="password-repeat-user" placeholder="Повторите пароль" />
                    ...
                    <ErrorMessage errors={errors} name="passwordRepeat" />
                    <p className={style["form-register__hint"]}>* поля обязательные для заполнения</p>
                    <div className={style["approval-block"]}>
                        <input type="checkbox" {...register("approval", { required: "Согласитесь с условиями" })} id="approval" />
                        <label htmlFor="approval">Я согласен с правилами <a href='/'>сайта</a> </label>
                    </div>
                    <ErrorMessage errors={errors} name="approval" />
                    <div className={style["register-submit-btn"]}>
                        <button id="btn-register" type="submit"
                            className={`${style["register__btn"]} ${style['form-lk-btn']} ${style.active}`}
                        >
                            Зарегистрироваться
                        </button>
                    </div>
